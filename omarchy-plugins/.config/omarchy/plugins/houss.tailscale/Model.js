...
function accountLabel(account) {
  ...
  // Tailscale defaults a profile's nickname to the login it was created with,
  // so one identity that belongs to several tailnets yields several profiles
  // labelled identically. Only treat a nickname as meaningful when it differs
  // from the login; otherwise fall back to the tailnet, which is the field
  // that actually distinguishes the profiles (and the one that
  // `tailscale switch --list` prints).
